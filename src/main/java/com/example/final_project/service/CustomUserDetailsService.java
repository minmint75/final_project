...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        if (teacherOpt.isPresent()) {
            ...
            if (teacher.getStatus() == Teacher.TeacherStatus.PENDING) {
                System.out.println("DEBUG: Teacher " + email + " is PENDING approval. Denying access.");
                throw new AccountNotApprovedException(
                        "Tài khoản giáo viên đang chờ phê duyệt. Vui lòng liên hệ quản trị viên để được kích hoạt.");
            }

            // Check if teacher account is rejected
            if (teacher.getStatus() == Teacher.TeacherStatus.REJECTED) {
                System.out.println("DEBUG: Teacher " + email + " is REJECTED. Denying access.");
                throw new AccountNotApprovedException(
                        "Tài khoản giáo viên đã bị từ chối. Vui lòng liên hệ quản trị viên để biết thêm chi tiết.");
            }
            ...
            return buildUserDetails(teacher.getTeacherId(), teacher.getEmail(), teacher.getPassword(),
                    RoleName.TEACHER);
        }
        ...
        if (studentOpt.isPresent()) {
            ...
            return buildUserDetails(student.getStudentId(), student.getEmail(), student.getPassword(),
                    RoleName.STUDENT);
        }
    ...
    private UserDetails buildUserDetails(Long id, String email, String hashedPassword, RoleName role) {
        ...
        System.out.println("DEBUG: Building CustomUserDetails for " + email + " with ID: " + id + " and Authority: "
                + roleWithPrefix);
